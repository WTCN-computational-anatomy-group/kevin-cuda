...
if (mxIsGPUArray(INPUTDENSEA) && mxIsGPUArray(INPUTDENSEB)) {
    ...
    if ( (numARows!= numBRows) && (numAColumns != numBColumns) ) {
         ...
         mexErrMsgIdAndTxt( "MATLAB:mexatexit:invalidInput",
                "Invalid input to MEX file, row/column numbers of dense matrix(first argument) must be equal to row/column numbers of dense matrix(second argument).");
             
    }
       double alpha= mxGetScalar(ALPHA);
       double beta = mxGetScalar(BETA);
  ...
  double const *d_A_dense;
   ...
   d_B_dense = (double const *)(mxGPUGetDataReadOnly(INPUTDENSEGPUB));   
	...
	//double *d_B_dense;  gpuErrchk(cudaMalloc(&d_B_dense, numBRows * numBColumns * sizeof(*d_B_dense)));
	//gpuErrchk(cudaMemcpy(d_B_dense, h_B_dense1, numBRows * numBColumns * sizeof(*d_B_dense), cudaMemcpyHostToDevice));
	cusparseSafeCall(cusparseDnnz(handle, CUSPARSE_DIRECTION_ROW, numBRows, numBColumns, descrA, d_B_dense, ldb, d_nnzPerVectorB, &nnzB));
   ...
   mxGPUArray * COL_B = mxGPUCreateGPUArray(1, (mwSize*) pivot_dimensCOL_B, mxINT32_CLASS, mxREAL, MX_GPU_DO_NOT_INITIALIZE);
    int  *d_B_ColIndices = (int *)mxGPUGetData(COL_B);
		...
		cusparseSafeCall(cusparseDdense2csr(handle, numBRows, numBColumns, descrA, d_B_dense, ldb, d_nnzPerVectorB, d_B, d_B_RowIndices, d_B_ColIndices));
...
// nnzTotalDevHostPtr points to host memory
   ...
   mxGPUArray * COL_C = mxGPUCreateGPUArray(1, (mwSize*) pivot_dimensCOL_C, mxINT32_CLASS, mxREAL, MX_GPU_DO_NOT_INITIALIZE);
    int  *d_C_ColIndices = (int *)mxGPUGetData(COL_C);
   	...
   	cusparseSafeCall(cusparseDcsrgeam(handle, numARows, numBColumns, &alpha,
		descrA, nnzA,
...
////////////////////////////////////////////////////////////////////////////////////  
   ...
   h_B_dense1 = (double *)mxGetDoubles(INPUTDENSEB);    
	...
	//double *d_B_dense;  gpuErrchk(cudaMalloc(&d_B_dense, numBRows * numBColumns * sizeof(*d_B_dense)));
      ...
      mxGPUArray *OUTMB = mxGPUCreateGPUArray(2, (mwSize*) pivot_dimensionsvalueDB, mxDOUBLE_CLASS, mxREAL, MX_GPU_DO_NOT_INITIALIZE);
