...
if (mxIsGPUArray(INPUTSPARSEA) && mxIsGPUArray(INPUTSPARSEB)) {
    ...
    if ( (numARows!= numBRows) && (numAColumns != numBColumns) ) {
         ...
         mexErrMsgIdAndTxt( "MATLAB:mexatexit:invalidInput",
                "Invalid input to MEX file, row/column numbers of sparse matrix(first argument) must be equal to row/column numbers of sparse matrix(second argument).");
   ...
   mxArray *  COL_SORTB = mxCreateNumericMatrix(nnzB, 1, mxINT32_CLASS, mxREAL);
    ...
    cuDoubleComplex  *VALSORTB = (cuDoubleComplex *)mxGetComplexDoubles(VLSXY2);
   ...
   mxGPUArray * COL_B = mxGPUCreateGPUArray(1, (mwSize*) pivot_dimensCOL_B, mxINT32_CLASS, mxREAL, MX_GPU_DO_NOT_INITIALIZE);
	...
	cusparseSafeCall(cusparseZgthr(handle, nnzB, d_B, d_B, Pb, CUSPARSE_INDEX_BASE_ZERO));
...
////////////////////////////////////////////////////////////////////////////////////  
  ...
  //  }
   ...
   mxArray *  COL_SORTB = mxCreateNumericMatrix(nnzB, 1, mxINT32_CLASS, mxREAL);
    ...
    cuDoubleComplex  *VALSORTB = (cuDoubleComplex *)mxGetComplexDoubles(INPUTSPARSEB);
