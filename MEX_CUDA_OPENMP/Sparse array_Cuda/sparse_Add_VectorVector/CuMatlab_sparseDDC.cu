...
if (mxIsGPUArray(INPUTDENSEA) && mxIsGPUArray(INPUTDENSEB)) {
    ...
    if ( mxGPUGetNumberOfElements(INPUTDENSEGPUB)!=mxGPUGetNumberOfElements(INPUTDENSEGPUA)) {
              mxGPUDestroyGPUArray(INPUTDENSEGPUB);
              mxGPUDestroyGPUArray(INPUTDENSEGPUA);   
         mexErrMsgIdAndTxt( "MATLAB:mexatexit:invalidInput",
...
if (numAColumns == 1) {    
  ...
  cusparseSafeCall(cusparseZaxpyi( handle,  nnzA, 
               &alpha, 
...
if (numBColumns == 1) {	
    ...
    VAL = mxGPUCreateGPUArray(1, (mwSize*) pivot_dimensionsvalueV, mxDOUBLE_CLASS, mxCOMPLEX, MX_GPU_DO_NOT_INITIALIZE);
    VALOUT = (cuDoubleComplex *)mxGPUGetData(VAL);
       gpuErrchk(cudaMemcpy(VALOUT, h_B_dense1, sizeof(cuDoubleComplex) * numBRows , cudaMemcpyHostToDevice));
    }  
