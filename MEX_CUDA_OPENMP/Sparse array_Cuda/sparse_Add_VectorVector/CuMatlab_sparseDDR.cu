...
if (mxIsGPUArray(INPUTDENSEA) && mxIsGPUArray(INPUTDENSEB)) {
    ...
    if ( mxGPUGetNumberOfElements(INPUTDENSEGPUA)!=mxGPUGetNumberOfElements(INPUTDENSEGPUB)) {
     ...
     if ( (numARows!=numBRows)&& (numAColumns!=numBColumns)  ) {
              ...
              mxGPUDestroyGPUArray(INPUTDENSEGPUA);   
         mexErrMsgIdAndTxt( "MATLAB:mexatexit:invalidInput",
                        "Invalid input to MEX file, dense vector (first argument) and  dense vector(second argument) must be both row or column vectors.");
...
if (numAColumns == 1) {    
    ...
    VALOUT = (double *)mxGPUGetData(VAL);
       gpuErrchk(cudaMemcpy(VALOUT, d_B_dense, sizeof(double) * numARows , cudaMemcpyDeviceToDevice));
  cusparseSafeCall(cusparseDaxpyi( handle,  nnzA, 
               &alpha, 
               d_value_sort, d_row_sort, 
...
if (numARows == 1) {
    ...
    VALOUT = (double *)mxGPUGetData(VAL);
       gpuErrchk(cudaMemcpy(VALOUT, d_B_dense, sizeof(double) * numBColumns , cudaMemcpyDeviceToDevice));
  cusparseSafeCall(cusparseDaxpyi( handle,  nnzA, 
               &alpha, 
               d_value_sort, d_col_sort, 
               VALOUT, CUSPARSE_INDEX_BASE_ONE));		
...
if (numBColumns == 1) {	
  ...
  VAL = mxGPUCreateGPUArray(1, (mwSize*) pivot_dimensionsvalueV, mxDOUBLE_CLASS, mxREAL, MX_GPU_DO_NOT_INITIALIZE);
    VALOUT = (double *)mxGPUGetData(VAL);
       gpuErrchk(cudaMemcpy(VALOUT, h_B_dense1, sizeof(double) * numBRows , cudaMemcpyHostToDevice));
    }  
...
if (numBRows == 1) {
   ...
   VAL = mxGPUCreateGPUArray(2, (mwSize*) pivot_dimensionsvalueV, mxDOUBLE_CLASS, mxREAL, MX_GPU_DO_NOT_INITIALIZE);
    VALOUT = (double *)mxGPUGetData(VAL);
       gpuErrchk(cudaMemcpy(VALOUT, h_B_dense1, sizeof(double) * numBColumns , cudaMemcpyHostToDevice));
