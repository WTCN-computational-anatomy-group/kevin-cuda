...
if ((mxIsGPUArray(INPUTMATRIX))) {
   ...
   // if (mxGetClassID(mxGPUCreateMxArrayOnCPU(GPUINPUTMATRIX)) != mxDOUBLE_CLASS  && mxGetClassID(mxGPUCreateMxArrayOnCPU(GPUINPUTMATRIX)) != mxSINGLE_CLASS ) {
     //    mexErrMsgIdAndTxt( "MATLAB:mexatexit:invalidInput",
            //    "Invalid input to MEX file, input(FIRST ARGUMENT) must be single or cuDoubleComplex precision.");
   ...
   cusparseHandle_t handle;    cusparseSafeCall(cusparseCreate(&handle));
	//double *d_A_denseReconstructed; gpuErrchk(cudaMalloc(&d_A_denseReconstructed, numARows * numAColumns * sizeof(double)));
        ...
        cuDoubleComplex *d_A_denseReconstructed = (cuDoubleComplex*)mxGPUGetData(denseReconstructed);
    ...
    if(!mxIsSparse(INPUTMATRIX)) {
     ...
     // if (mxGetClassID(INPUTMATRIX) != mxDOUBLE_CLASS  && mxGetClassID(INPUTMATRIX) != mxSINGLE_CLASS ) {
       //  mexErrMsgIdAndTxt( "MATLAB:mexatexit:invalidInput",
        //        "Invalid input to MEX file, input(FIRST ARGUMENT) must be single or cuDoubleComplex precision.");
   ...
   // }
    ...
    OUPUTMATRIX = mxCreateNumericMatrix(numARows, numAColumns, mxDOUBLE_CLASS, mxCOMPLEX);
    cuDoubleComplex  *h_A_denseReconstructed = (cuDoubleComplex *)mxGetComplexDoubles(OUPUTMATRIX);
   ...
   mxGPUArray * COL_SORT1 = mxGPUCreateGPUArray(1, (mwSize*) pivot_dimensionscolumn, mxINT32_CLASS, mxREAL, MX_GPU_DO_NOT_INITIALIZE);
    ...
    cuDoubleComplex *d_A = (cuDoubleComplex *)mxGPUGetData(VAL_SORT1);
		...
		cusparseSafeCall(cusparseZcsc2dense(handle, numARows, numAColumns, descrA, d_A, d_A_RowIndices, d_A_ColIndices,
			d_A_denseReconstructed, numARows));
		//gpuErrchk(cudaMemcpy(h_A_denseReconstructed, d_A_denseReconstructed, numARows * numAColumns * sizeof(cuDoubleComplex), cudaMemcpyDeviceToHost));
