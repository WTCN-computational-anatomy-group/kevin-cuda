...
if (mxIsGPUArray(INPUTDENSEA) && mxIsGPUArray(INPUTDENSEB)) {
    ...
    if ( (((numARows!= 1) && (numAColumns!= 1))) ||(((numBRows!= 1) && (numBColumns!= 1)))) {
         ...
         mexErrMsgIdAndTxt( "MATLAB:mexatexit:invalidInput",
                         "Invalid input to MEX file, first/second arguments must be dense/dense column/row vector");
...
//gpuErrchk(cudaFree(d_A_RowIndices));
  ...
  if (numAColumns == 1) { 
  cusparseSafeCall(cusparseZdotci(handle, nnzA, 
              d_value_sort, 
 ...
 if (numARows == 1) {
  ...
  OUTPUTMATRIX = mxCreateDoubleMatrix(1, 1, mxCOMPLEX);
// *mxGetPr(OUTPUTMATRIX) = VALOUT;
//cuDoubleComplex *value= (cuDoubleComplex *)mxGetComplexDoubles(OUTPUTMATRIX);
(*mxGetComplexDoubles(OUTPUTMATRIX)).real = VALOUT.x;
(*mxGetComplexDoubles(OUTPUTMATRIX)).imag = VALOUT.y;
...
if (numBColumns == 1) {	
  ...
  VAL = mxGPUCreateGPUArray(1, (mwSize*) pivot_dimensionsvalueV, mxDOUBLE_CLASS, mxCOMPLEX, MX_GPU_DO_NOT_INITIALIZE);
    VALDENSE = (cuDoubleComplex *)mxGPUGetData(VAL);
       gpuErrchk(cudaMemcpy(VALDENSE, h_B_dense1, sizeof(cuDoubleComplex) * numBRows , cudaMemcpyHostToDevice));
    }  
...
if (numBRows == 1) {
   ...
   VAL = mxGPUCreateGPUArray(1, (mwSize*) pivot_dimensionsvalueV, mxDOUBLE_CLASS, mxCOMPLEX, MX_GPU_DO_NOT_INITIALIZE);
    VALDENSE = (cuDoubleComplex *)mxGPUGetData(VAL);
       gpuErrchk(cudaMemcpy(VALDENSE, h_B_dense1, sizeof(cuDoubleComplex) * numBColumns , cudaMemcpyHostToDevice));
