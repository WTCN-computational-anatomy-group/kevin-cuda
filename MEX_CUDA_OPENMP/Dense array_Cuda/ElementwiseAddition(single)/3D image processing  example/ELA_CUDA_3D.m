%
 % Three dimensional Element wise Addition
 % 
 ...
 % gpuArray output, C=ELM_CUDA_3D(A,B,alpha, beta) C=A*alpha+B*beta.
 ...
 function OUTCoefs = ELA_CUDA_3D(data1,data2, alpha, beta)
...
OUTCoefs=single(zeros(ex1,wy1,zy1, 'gpuArray'));

for i=1:zy1
    tempData1=gpuArray(data1(:,:,i));
    tempData2=gpuArray(data2(:,:,i));
    OUTCoefs(:,:,i)=ELA_CUDA(tempData1,tempData2, alpha, beta);
...
OUTCoefs=single(zeros(ex1,wy1,zy1));
