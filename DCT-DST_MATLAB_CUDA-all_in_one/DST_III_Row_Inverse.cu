...
__global__ void DSTIII_Row__InverseKernel(double  *A, double  *C,
	...
	int numAColumns, int numCRows, int numCColumns)
{
	...
	DSTIII_Row__InverseKernel << <dimGrid, dimBlock >> >(deviceA, deviceC, numARows, numAColumns, numCRows, numCColumns);
