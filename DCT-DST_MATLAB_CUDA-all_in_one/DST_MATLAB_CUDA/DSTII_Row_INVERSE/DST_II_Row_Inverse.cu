...
__global__ void DSTII_Row__InverseKernel_GPUA(double const * const A, double const * const B, double * const C,
...
__global__ void DSTII_Row__InverseKernel(double  *A, double   *B, double  *C,
	int numARows, int numAColumns,
	int numBRows, int numBColumns,
	int numCRows, int numCColumns)
{
	...
	int numCRows, int numCColumns)
{
	...
	DSTII_Row__InverseKernel << <dimGrid, dimBlock >> >(deviceA, deviceB, deviceC, numARows, numAColumns, numBRows, numBColumns, numCRows, numCColumns);
...
// 		}
  ...
  //  DCOS=mxGPUCreateFromMxArray(hostcos);
	...
	dimGrid.x = (numCColumns + dimBlock.x - 1) / dimBlock.x;
	dimGrid.y = (numCRows + dimBlock.y - 1) / dimBlock.y;
   DSTII_Row__InverseKernel_GPUA<< <dimGrid, dimBlock >> >(d_A, d_DCOS, d_B, numARows, numAColumns, numDCOSRows, numDCOSColumns, numCRows, numCColumns);
