...
__global__ void DSTIII_Column_Kernel_GPUA(double const * const A, double const * const B, double * const C,
...
__global__ void DSTIII_Column_Kernel(double  *A, double   *B, double  *C,
	int numARows, int numAColumns,
	int numBRows, int numBColumns,
	int numCRows, int numCColumns)
{
	...
	int numCRows, int numCColumns)
{
	...
	DSTIII_Column_Kernel << <dimGrid, dimBlock >> >(deviceA, deviceB, deviceC, numARows, numAColumns, numBRows, numBColumns, numCRows, numCColumns);
...
// 				[i* numDCOSColumns + j] = cos(((2 * i + 1) / (2.0 * numDCOSColumns))*PI_d*j)*sqrt(1.0 / (1 + DELTA(1, j + 1)))*sqrt(2.0 / numDCOSColumns);
// 				pointer//hostB[i + j* numBColumns] = 1;
// 				
// 					//hostL[i* numBColumns + j] = cos(((2 * j + 1) / (2.0 * numBColumns))*3.14*i)*sqrt(1.0 / numBColumns);
// 					//hostB[i + j* numBColumns] = 1;
// 			
...
// 		}
  ...
  //  DCOS=mxGPUCreateFromMxArray(hostcos);
   ...
   DSTIII_Column_Kernel_GPUA<< <dimGrid, dimBlock >> >(d_DCOS, d_A, d_B, numDCOSRows, numDCOSColumns, numARows, numAColumns, numCRows, numCColumns);
