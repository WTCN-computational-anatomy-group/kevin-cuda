/*
 * Inverse Discrete Sine Transform in Column wise (DST two)
 * DST_II_Column_Inverse
 ...
 * gpuArray output, B=DST_II_Column_Inverse(A)=mexFunction(A).
...
__global__ void DSTII_Column_Inverse_Kernel_GPUA(double const * const A, double const * const B, double * const C,
...
__global__ void DSTII_Column_Inverse_Kernel(double  *A, double   *B, double  *C,
	int numARows, int numAColumns,
	int numBRows, int numBColumns,
	int numCRows, int numCColumns)
{
	...
	int numCRows, int numCColumns)
{
	...
	DSTII_Column_Inverse_Kernel << <dimGrid, dimBlock >> >(deviceA, deviceB, deviceC, numARows, numAColumns, numBRows, numBColumns, numCRows, numCColumns);
...
// 		}
  ...
  //  DCOS=mxGPUCreateFromMxArray(hostcos);
   ...
   DSTII_Column_Inverse_Kernel_GPUA<< <dimGrid, dimBlock >> >(d_DCOS, d_A, d_B, numDCOSRows, numDCOSColumns, numARows, numAColumns, numCRows, numCColumns);
