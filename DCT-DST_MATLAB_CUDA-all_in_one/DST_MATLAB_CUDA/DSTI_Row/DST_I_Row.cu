...
__global__ void DSTI_Row_Kernel_GPUA(double const * const A, double const * const B, double * const C,
...
__global__ void DSTI_Row_Kernel(double  *A, double   *B, double  *C,
	int numARows, int numAColumns,
	int numBRows, int numBColumns,
	int numCRows, int numCColumns)
{
	...
	int numCRows, int numCColumns)
{
	...
	DSTI_Row_Kernel << <dimGrid, dimBlock >> >(deviceA, deviceB, deviceC, numARows, numAColumns, numBRows, numBColumns, numCRows, numCColumns);
...
pointer = mxGetPr(COS);
		...
		//Discrete Sine Transform in row wise  i=k-1  j=n-1
...
// 		}
  ...
  //  DCOS=mxGPUCreateFromMxArray(hostcos);
	...
	dimGrid.x = (numCColumns + dimBlock.x - 1) / dimBlock.x;
	dimGrid.y = (numCRows + dimBlock.y - 1) / dimBlock.y;
   DSTI_Row_Kernel_GPUA<< <dimGrid, dimBlock >> >(d_A, d_DCOS, d_B, numARows, numAColumns, numDCOSRows, numDCOSColumns, numCRows, numCColumns);
