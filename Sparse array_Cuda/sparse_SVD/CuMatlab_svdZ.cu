...
if (mxIsGPUArray(INPUTMATRIX)) {
   ...
   mxGPUArray * COL_SORT1 = mxGPUCreateGPUArray(1, (mwSize*) pivot_dimensionscolumn, mxINT32_CLASS, mxREAL, MX_GPU_DO_NOT_INITIALIZE);
    ...
    cuDoubleComplex  *d_A = (cuDoubleComplex *)mxGPUGetData(VAL_SORT1);
	...
	//cuDoubleComplex *d_A_denseReconstructed; gpuErrchk(cudaMalloc(&d_A_denseReconstructed, numARows * numAColumns * sizeof(cuDoubleComplex)));
    mxGPUArray *denseReconstructed = mxGPUCreateGPUArray(2, dimsGPU, mxDOUBLE_CLASS, mxCOMPLEX, MX_GPU_DO_NOT_INITIALIZE);
    cuDoubleComplex *d_A_dense = (cuDoubleComplex*)mxGPUGetData(denseReconstructed);
		//cuDoubleComplex *d_A;            gpuErrchk(cudaMalloc(&d_A, nnz * sizeof(*d_A)));
		...
		cusparseSafeCall(cusparseZcsc2dense(handle, numARows, numAColumns, descrA, d_A, d_A_RowIndices, d_A_ColIndices,
			d_A_dense, numARows));
	...
	//cusparseSetMatIndexBase(descrA, CUSPARSE_INDEX_BASE_ONE);
      ...
      mxGPUArray *U = mxGPUCreateGPUArray(2, (mwSize*) pivot_dimensionsleftunitarymatrix, mxDOUBLE_CLASS, mxCOMPLEX, MX_GPU_DO_NOT_INITIALIZE);
      cuDoubleComplex *d_U = (cuDoubleComplex *)mxGPUGetData(U);
    
      size_t pivot_dimensionsrightunitarymatrix[2] = {numAColumns, numAColumns};
      mxGPUArray *V = mxGPUCreateGPUArray(2, (mwSize*) pivot_dimensionsrightunitarymatrix, mxDOUBLE_CLASS, mxCOMPLEX, MX_GPU_DO_NOT_INITIALIZE);
      cuDoubleComplex *d_V = (cuDoubleComplex *)mxGPUGetData(V);	
      ...
      cusolverSafeCall(cusolverDnZgesvd_bufferSize(cusolver_handle, numARows, numAColumns, &lwork));
      
      size_t pivot_dimensionswork[1] = {lwork};
      mxGPUArray *W = mxGPUCreateGPUArray(1, (mwSize*) pivot_dimensionswork, mxDOUBLE_CLASS, mxCOMPLEX, MX_GPU_DO_NOT_INITIALIZE);
      cuDoubleComplex *work = (cuDoubleComplex *)mxGPUGetData(W);
    ...
    cusolverSafeCall(cusolverDnZgesvd(cusolver_handle, 'A', 'A', numARows, numAColumns, d_A_dense, numARows,
   ...
   if ( numARows*numAColumns < mediumsizematrix ) { 
    ...
    cusolverSafeCall(cusolverDnZgesvdj_bufferSize(
    cusolver_handle,
    ...
    cusolverSafeCall(cusolverDnZgesvdj(
    cusolver_handle,
    CUSOLVER_EIG_MODE_VECTOR, 
    econ,             
    numARows,                
  ...
  if ( numARows*numAColumns < mediumsizematrix ) { 
	...
	//cusparseDestroy(handle);
      ...
      //   mexErrMsgIdAndTxt( "MATLAB:mexatexit:invalidInput",
         //       "Invalid input to MEX file, input(FIRST ARGUMENT) must be  cuDoubleComplex precision.");
...
if(mxIsSparse(INPUTMATRIX)) {
  ...
  if ( numARows < numAColumns ) {
    ...
    int nnz= static_cast<int> (nnz1);
    cusparseHandle_t handle;    cusparseSafeCall(cusparseCreate(&handle));
    cuDoubleComplex * h_A=(cuDoubleComplex *)mxGetComplexDoubles(INPUTMATRIX);
