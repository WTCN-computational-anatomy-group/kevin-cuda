...
if (numAColumns == 1) {	
    ...
    VALOUT = (cuDoubleComplex *)mxGPUGetData(VAL);
       gpuErrchk(cudaMemcpy(VALOUT, d_A_dense, sizeof(cuDoubleComplex) * numARows , cudaMemcpyDeviceToDevice));
    }  
...
if (numARows == 1) {
   ...
   size_t  pivot_dimensionsvalueV[2] = {1,numBColumns};
    ...
    VALOUT = (cuDoubleComplex *)mxGPUGetData(VAL);
       gpuErrchk(cudaMemcpy(VALOUT, d_A_dense, sizeof(cuDoubleComplex) * numBColumns , cudaMemcpyDeviceToDevice));
...
if (numAColumns == 1) {	
    ...
    VAL = mxGPUCreateGPUArray(1, (mwSize*) pivot_dimensionsvalueV, mxDOUBLE_CLASS, mxCOMPLEX, MX_GPU_DO_NOT_INITIALIZE);
    VALOUT = (cuDoubleComplex *)mxGPUGetData(VAL);
       gpuErrchk(cudaMemcpy(VALOUT, h_A_dense1, sizeof(cuDoubleComplex) * numARows , cudaMemcpyHostToDevice));
    }  
...
if (numARows == 1) {
   ...
   size_t  pivot_dimensionsvalueV[2] = {1,numBColumns};
    VAL = mxGPUCreateGPUArray(2, (mwSize*) pivot_dimensionsvalueV, mxDOUBLE_CLASS, mxCOMPLEX, MX_GPU_DO_NOT_INITIALIZE);
    VALOUT = (cuDoubleComplex *)mxGPUGetData(VAL);
       gpuErrchk(cudaMemcpy(VALOUT, h_A_dense1, sizeof(cuDoubleComplex) * numBColumns , cudaMemcpyHostToDevice));
