...
#ifndef _ELA_CUDA_KERNEL_H_
#define _ELA_CUDA_KERNEL_H_
...
template <unsigned int TILE_DIM > __global__ void MatAddSharedElementwise_GPUA(float const * const A, float const * const B, float * const C,
	int numARows, int numAColumns, int numBRows, int numBColumns,
	int numCRows, int numCColumns, float alpha, float beta)
{
	float CValue = 0.0;
...
#endif // #ifndef _ELA_CUDA_KERNEL_H_
