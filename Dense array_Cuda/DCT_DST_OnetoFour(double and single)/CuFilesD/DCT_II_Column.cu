...
template <unsigned int TILE_DIM >  __global__ void DCTII_Column_Kernel(double  *A, double  *C,
	...
	int numCRows, int numCColumns)
{
	...
	for (int k = 0; k < (TILE_DIM + numARows - 1) / TILE_DIM; k++) {
        // need to be changed sqrt ---> sqrtf to have more accur.
        // need to be changed   cos --->  __cosf  faster
        // need to be changed   sin  --->  __sinf   faster
		if (k*TILE_DIM + threadIdx.x < numARows && Row < numARows)	{ As[threadIdx.y][threadIdx.x] = cos(((2 * (threadIdx.x + k*TILE_DIM) + 1) / (2.0 * numARows))*PI_d*Row)*sqrt(1.0 / (1 + DELTA(Row + 1, 1)))*sqrt(2.0 / numARows); }
                                                                                                    //cos(((2 * (threadIdx.y + k*TILE_DIM) + 1) / (2.0 * numAColumns))*PI_d*Col)*sqrt(1.0 / (1 + DELTA(Col + 1, 1)))*sqrt(2.0 / numAColumns);
	...
	int numAColumns, int numCRows, int numCColumns)
{
  ...
  switch (TILEDIM){
	...
	DCTII_Column_Kernel <16> << <dimGrid, dimBlock >> >(deviceA, deviceC, numARows, numAColumns, numCRows, numCColumns);
	...
	dimGrid.x = (numCColumns + dimBlock.x - 1) / dimBlock.x;
	dimGrid.y = (numCRows + dimBlock.y - 1) / dimBlock.y;
		DCTII_Column_Kernel <32> << <dimGrid, dimBlock >> >(deviceA, deviceC, numARows, numAColumns, numCRows, numCColumns);
	//matrixMultiplyShared << <dimGrid, dimBlock >> >(thrust::raw_pointer_cast(&dev_ptr_A[0]), thrust::raw_pointer_cast(&dev_ptr_C[0]), numARows, numAColumns, numCRows, numCColumns);
	gpuErrchk(cudaPeekAtLastError());
	gpuErrchk(cudaDeviceSynchronize());
