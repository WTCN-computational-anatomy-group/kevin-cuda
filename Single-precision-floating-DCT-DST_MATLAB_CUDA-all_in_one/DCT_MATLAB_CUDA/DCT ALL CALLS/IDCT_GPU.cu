/*
 * Inverse Discrete Cosine Transform(IDCT one to four)
 * IDCT I ---> IV
 ...
 * gpuArray output, B=IDCT_GPU(A, type of Inverse DCT, dimensions).
...
__global__ void IDCT_GPUA(double const * const A, double const * const B, double * const C,
...
__global__ void IDCT_Kernel(double  *A, double   *B, double  *C,
	int numARows, int numAColumns,
	int numBRows, int numBColumns,
	int numCRows, int numCColumns)
{
	...
	int numCRows, int numCColumns)
{
	...
	IDCT_Kernel << <dimGrid, dimBlock >> >(deviceA, deviceB, deviceC, numARows, numAColumns, numBRows, numBColumns, numCRows, numCColumns);
...
if (strcmp (four,input_buf1) == 0){ 
  ...
  //  DCOS=mxGPUCreateFromMxArray(hostcos);
	...
	dimGrid.x = (numCColumns + dimBlock.x - 1) / dimBlock.x;
	dimGrid.y = (numCRows + dimBlock.y - 1) / dimBlock.y;
   IDCT_GPUA<< <dimGrid, dimBlock >> >(d_A, d_DCOS, d_B, numARows, numAColumns, numDCOSRows, numDCOSColumns, numCRows, numCColumns);
...
if (strcmp (four,input_buf1) == 0){
  ...
  //  DCOS=mxGPUCreateFromMxArray(hostcos);
   ...
   IDCT_GPUA<< <dimGrid, dimBlock >> >(d_DCOS, d_A, d_B, numDCOSRows, numDCOSColumns, numARows, numAColumns, numCRows, numCColumns);
