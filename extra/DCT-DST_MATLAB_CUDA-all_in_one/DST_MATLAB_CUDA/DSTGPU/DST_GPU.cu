/*
 * Discrete Sine Transform(DST one to four)
 * DST I ---> IV
 ...
 * gpuArray output, B=DST_GPU(A, type of DST, dimensions).
...
__global__ void DST_GPUA(double const * const A, double const * const B, double * const C,
...
__global__ void DST_Kernel(double  *A, double   *B, double  *C,
	int numARows, int numAColumns,
	int numBRows, int numBColumns,
	int numCRows, int numCColumns)
{
	...
	int numCRows, int numCColumns)
{
	...
	DST_Kernel << <dimGrid, dimBlock >> >(deviceA, deviceB, deviceC, numARows, numAColumns, numBRows, numBColumns, numCRows, numCColumns);
 ...
 if (strcmp (one,input_buf1) == 0){
		for (int i = 0; i < numDCOSRows; i++){
			for (int j = 0; j < numDCOSColumns; j++){
				...
				//hostB[i + j* numBColumns] = cos((j*3.14*i / (numBColumns - 1)))*sqrt(1.0 / (1 + DELTA(i + 1, 1) + DELTA(i + 1, numBRows)))*sqrt(1.0 / (1 + DELTA(1, j + 1) + DELTA(numBColumns, j + 1)))*sqrt(2.0 / numBColumns);
 ...
 if (strcmp (two,input_buf1) == 0){
		for (int i = 0; i < numDCOSRows; i++){
			for (int j = 0; j < numDCOSColumns; j++){
				...
				//cosvalx[i * numBColumns + j] = cos(((2 * j + 1) / (2.0 * numBColumns))*3.14*i)*sqrt(1.0 / numBColumns);
				//hostB[i * numBColumns + j] = cosvalx[i + j* numAColumns];
				//hostB[i + j* numBColumns] = sin((((j+1)*3.14*(i+1)) / (numBColumns + 1)))*sqrt(2.0 / (numBColumns+1));// DST I
...
if (strcmp (four,input_buf1) == 0){
		for (int i = 0; i < numDCOSRows; i++){
			for (int j = 0; j < numDCOSColumns; j++){
				...
				//hostB[i * numBColumns + j] = cosvalx[i + j* numAColumns];
				//hostB[i + j* numBColumns] = sin((((j+1)*3.14*(i+1)) / (numBColumns + 1)))*sqrt(2.0 / (numBColumns+1));// DST I
				//hostB[i + j* numBColumns] = sin(((j + 1)*PI_d*(i + 0.5)) / (numBColumns))*sqrt(2.0 / (numBColumns))*sqrt(1.0 / (1 + DELTA(numBColumns, j + 1))); // DST II and III  
  ...
  //  DCOS=mxGPUCreateFromMxArray(hostcos);
	...
	dimGrid.x = (numCColumns + dimBlock.x - 1) / dimBlock.x;
	dimGrid.y = (numCRows + dimBlock.y - 1) / dimBlock.y;
   DST_GPUA<< <dimGrid, dimBlock >> >(d_A, d_DCOS, d_B, numARows, numAColumns, numDCOSRows, numDCOSColumns, numCRows, numCColumns);
...
//	}
	...
	//Discrete Sine Transform in row wise
 ...
 if (strcmp (two,input_buf1) == 0){
		for (int i = 0; i < numDCOSRows; i++){
			for (int j = 0; j < numDCOSColumns; j++){
				...
				//hostB[i * numBColumns + j] = cosvalx[i + j* numAColumns];
				//hostB[i + j* numBColumns] = sin((((j + 1)*3.14*(i + 1)) / (numBColumns + 1)))*sqrt(2.0 / (numBColumns + 1));
				//hostL[i* numBColumns + j] = sin((((j + 1)*3.14*(i + 1)) / (numBColumns + 1)))*sqrt(2.0 / (numBColumns + 1));  //DST I  Column
				//hostB[i + j* numBColumns] = sin(((j + 0.5)*3.14*(i + 1)) / (numBColumns))*sqrt((2.0 - DELTA(i + 1, numBRows)) / (numBColumns));
...
if (strcmp (four,input_buf1) == 0){
		for (int i = 0; i < numDCOSRows; i++){
			for (int j = 0; j < numDCOSColumns; j++){
				...
				//hostL[i* numBColumns + j] = sin((((j + 1)*3.14*(i + 1)) / (numBColumns + 1)))*sqrt(2.0 / (numBColumns + 1));  //DST I  Column
				//hostB[i + j* numBColumns] = sin(((j + 0.5)*3.14*(i + 1)) / (numBColumns))*sqrt((2.0 - DELTA(i + 1, numBRows)) / (numBColumns));
				//hostL[i* numBColumns + j] = sin(((j + 1)*PI_d*(i + 0.5)) / (numBColumns))*sqrt(2.0 / (numBColumns))*sqrt(1.0 / (1 + DELTA(numBColumns, j + 1)));  //DST II and III  Column
  ...
  //  DCOS=mxGPUCreateFromMxArray(hostcos);
   ...
   DST_GPUA<< <dimGrid, dimBlock >> >(d_DCOS, d_A, d_B, numDCOSRows, numDCOSColumns, numARows, numAColumns, numCRows, numCColumns);
