/*
 * Inverse Discrete Cosine Transform in row wise (DCT four)
 * DCT_IV_Row_Inverse
 ...
 * gpuArray output, B=DCT_IV_Row_Inverse(A)=mexFunction(A).
...
__global__ void DCTIV_Row__InverseKernel_GPUA(double const * const A, double const * const B, double * const C,
...
__global__ void DCTIV_Row__InverseKernel(double  *A, double   *B, double  *C,
	int numARows, int numAColumns,
	int numBRows, int numBColumns,
	int numCRows, int numCColumns)
{
	...
	int numCRows, int numCColumns)
{
	...
	DCTIV_Row__InverseKernel << <dimGrid, dimBlock >> >(deviceA, deviceB, deviceC, numARows, numAColumns, numBRows, numBColumns, numCRows, numCColumns);
...
//Inverse Discrete Cosine Transform in row wise
...
// 			//hostBinv[i * numBColumns + j] = 1;
// 			if (j == 0) {
// 				pointer[i + j* numDCOSColumns] = cos(((2 * i + 1) / (2.0 * numDCOSColumns))*PI_d*j)*sqrt(1.0 / numDCOSColumns);
// 			}
// 			else if (j != 0) {
// 				pointer[i + j* numDCOSColumns] = cos(((2 * j + 1) / (2.0 * numDCOSColumns))*PI_d*i)*sqrt(2.0 / numDCOSColumns);
// 			}
// 
// 		}
// 	}
...
// 			//hostcos[i * numBColumns + j] = cosvalx[i + j* numAColumns];
// 			if (i == 0) {
// 				//pointer[i + j* numDCOSColumns] = cos(((2 * j + 1) / (2.0 * numDCOSColumns))*3.14*i)*sqrt(1.0 / numDCOSColumns);
//                 //test ok
//                 pointer[i + j* numDCOSColumns] = 1;
// 			}
// 			else if (i != 0) {
// 				//pointer[i + j* numDCOSColumns] = cos(((2 * j + 1) / (2.0 * numDCOSColumns))*3.14*i)*sqrt(2.0 / numDCOSColumns);
//                  //test ok
...
// 	} 
  ...
  //  DCOS=mxGPUCreateFromMxArray(hostcos);
	...
	dimGrid.x = (numCColumns + dimBlock.x - 1) / dimBlock.x;
	dimGrid.y = (numCRows + dimBlock.y - 1) / dimBlock.y;
   DCTIV_Row__InverseKernel_GPUA<< <dimGrid, dimBlock >> >(d_A, d_DCOS, d_B, numARows, numAColumns, numDCOSRows, numDCOSColumns, numCRows, numCColumns);
...
// 			//hostBinv[i * numBColumns + j] = 1;
// 			if (j == 0) {
// 				hostB[i + j* numBColumns] = cos(((2 * i + 1) / (2.0 * numBColumns))*PI_d*j)*sqrt(1.0 / numBColumns);
// 			}
// 			else if (j != 0) {
// 				hostB[i + j* numBColumns] = cos(((2 * j + 1) / (2.0 * numBColumns))*PI_d*i)*sqrt(2.0 / numBColumns);
// 			}
// 
// 		}
// 	}
...
// 			if (i == 0) {
// 				//hostB[i + j* numBColumns] = cos(((2 * j + 1) / (2.0 * numBColumns))*3.14*i)*sqrt(1.0 / numBColumns);
// 				hostB[i + j* numBColumns] = 1;
// 			}
// 			else if (i != 0) {
// 				//hostB[i + j* numBColumns] = cos(((2 * j + 1) / (2.0 * numBColumns))*3.14*i)*sqrt(2.0 / numBColumns);
