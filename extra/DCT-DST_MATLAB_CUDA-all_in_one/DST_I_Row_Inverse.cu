...
__global__ void DSTI_Row__InverseKernel(double  *A, double  *C,
	...
	int numAColumns, int numCRows, int numCColumns)
{
	...
	DSTI_Row__InverseKernel << <dimGrid, dimBlock >> >(deviceA, deviceC, numARows, numAColumns, numCRows, numCColumns);
