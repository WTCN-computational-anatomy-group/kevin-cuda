/*
 * Inverse Discrete Cosine Transform in Column wise (DCT two)
 * DCT_II_Column_Inverse
 ...
 * gpuArray output, B=DCT_II_Column_Inverse(A)=mexFunction(A).
...
__global__ void DCTII_Column_Inverse_Kernel_GPUA(double const * const A, double const * const B, double * const C,
...
__global__ void DCTII_Column_Inverse_Kernel(double  *A, double   *B, double  *C,
	int numARows, int numAColumns,
	int numBRows, int numBColumns,
	int numCRows, int numCColumns)
{
	...
	int numCRows, int numCColumns)
{
	...
	DCTII_Column_Inverse_Kernel << <dimGrid, dimBlock >> >(deviceA, deviceB, deviceC, numARows, numAColumns, numBRows, numBColumns, numCRows, numCColumns);
...
pointer = mxGetPr(COS);
		...
		for (int i = 0; i < numDCOSRows; i++){
			for (int j = 0; j < numDCOSColumns; j++){
				...
				//hostBinvL[i* numBColumns + j] = cos(((2 * j + 1) / (2.0 * numBColumns))*3.14*i)*sqrt(1.0 / (1 + DELTA(i + 1, 1)))*sqrt(2.0 / numBColumns);
...
// 				//hostBinv[i * numDCOSColumns + j] = cosval2x[i + j* numAColumns];
// 				//hostBinv[i * numBColumns + j] = 1;
// 				if (j == 0) {
// 					pointer[i* numDCOSColumns + j] = cos(((2 * i + 1) / (2.0 * numDCOSColumns))*PI_d*j)*sqrt(1.0 / numDCOSColumns);
// 				}
// 				else if (j != 0) {
...
// 		}
  ...
  //  DCOS=mxGPUCreateFromMxArray(hostcos);
   ...
   DCTII_Column_Inverse_Kernel_GPUA<< <dimGrid, dimBlock >> >(d_DCOS, d_A, d_B, numDCOSRows, numDCOSColumns, numARows, numAColumns, numCRows, numCColumns);
