/*
 * Discrete Cosine Transform in Column wise (DCT four)
 * DCT_IV_Column
 ...
 * gpuArray output, B=DCT_IV_Column(A)=mexFunction(A).
...
__global__ void DCTIV_Column_Kernel_GPUA(double const * const A, double const * const B, double * const C,
...
__global__ void DCTIV_Column_Kernel(double  *A, double   *B, double  *C,
	int numARows, int numAColumns,
	int numBRows, int numBColumns,
	int numCRows, int numCColumns)
{
	...
	int numCRows, int numCColumns)
{
	...
	DCTIV_Column_Kernel << <dimGrid, dimBlock >> >(deviceA, deviceB, deviceC, numARows, numAColumns, numBRows, numBColumns, numCRows, numCColumns);
...
// 			if (i == 0) {
// 				pointer[i + j* numDCOSColumns] = cos(((2 * j + 1) / (2.0 * numDCOSColumns))*PI_d*i)*sqrt(1.0 / numDCOSColumns);
//                 
//                // pointer[i + j* numDCOSColumns] = 1;
// 			}
// 			else if (i != 0) {
...
// 	} 
  ...
  //  DCOS=mxGPUCreateFromMxArray(hostcos);
   ...
   DCTIV_Column_Kernel_GPUA<< <dimGrid, dimBlock >> >(d_DCOS, d_A, d_B, numDCOSRows, numDCOSColumns, numARows, numAColumns, numCRows, numCColumns);
...
// 			//hostB[i * numBColumns + j] = cosvalx[i + j* numAColumns];
// 			if (i == 0) {
// 				hostB[i + j* numBColumns] = cos(((2 * j + 1) / (2.0 * numBColumns))*PI_d*i)*sqrt(1.0 / numBColumns);
// 				//hostB[i + j* numBColumns] = 1;
// 			}
// 			else if (i != 0) {
