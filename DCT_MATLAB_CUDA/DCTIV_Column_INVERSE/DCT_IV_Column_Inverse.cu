...
__global__ void DCTIV_Column_Inverse_Kernel_GPUA(double const * const A, double const * const B, double * const C,
...
__global__ void DCTIV_Column_Inverse_Kernel(double  *A, double   *B, double  *C,
	int numARows, int numAColumns,
	int numBRows, int numBColumns,
	int numCRows, int numCColumns)
{
	...
	int numCRows, int numCColumns)
{
	...
	DCTIV_Column_Inverse_Kernel << <dimGrid, dimBlock >> >(deviceA, deviceB, deviceC, numARows, numAColumns, numBRows, numBColumns, numCRows, numCColumns);
...
//		}
  ...
  //  DCOS=mxGPUCreateFromMxArray(hostcos);
   ...
   DCTIV_Column_Inverse_Kernel_GPUA<< <dimGrid, dimBlock >> >(d_DCOS, d_A, d_B, numDCOSRows, numDCOSColumns, numARows, numAColumns, numCRows, numCColumns);
