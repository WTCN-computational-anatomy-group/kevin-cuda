...
if nargin==4 && isa(varargin{1},'char') && strcmp(varargin{1},'kernel'),
    ...
    %param s[0:3] Voxel sizes
      ...
      switch bnd
        ...
        Alpha= MembLambda*(Delta2^(-2));   
        Beta= MembLambda*(Delta1^(-2)-Delta2^(-2));
        ...
        Beta= MembLambda*(Delta3^(-2)-Delta2^(-2));    
        acz  = ((1:K)'-1)/K*pi;
        acz  = 2*(1-cos(acz));
        
        wcz  = Alpha*acz+Beta*acz;    
       ...
       elseif (MembLambda==0 && BendLambda~=0 && ElasticMu==0 && ElasticLambda==0)  % Bending energy model
        ...
        Beta=  4*BendLambda*(Delta1^(-2))*(Delta2^(-2)); 
        Gamma= 6*BendLambda*(Delta1^(-4))+Dispabs;
        Teta=  2*BendLambda*(Delta1^(-2))*(Delta2^(-2));
        Eta= (-1)*BendLambda*(Delta2^(-4))+BendLambda*(Delta1^(-4));
        Xe=4*BendLambda*(Delta2^(-4))-4*BendLambda*(Delta1^(-4));
        if isvector(F) % 1D
        ...
        acxyy=2*cos(ac);
        ac  = 2*(1-cos(ac));
        wc  = Xe*acxyy+Eta*acxy+Teta*acx+Beta*ac+Alpha*ac.^2; 
        F  = 1./(Gamma+wc);
        ...
        asxyy=2*cos(as);
        as  = 2*(1-cos(as));   
        ws  = Xe*asxyy+Eta*asxy+Teta*asx+Beta*as+Alpha*as.^2;
